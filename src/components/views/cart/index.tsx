...
const CartView = (props: PropType) => {
    const { products, cart, setToaster } = props;
    const getProduct = (id: string) => {
        const product = products.find((product) => product.id === id);
        return product;
    };
    const getOptionsSize = (id: string, selectedSize: string) => {
        const product = products.find((product) => product.id === id);
                ...
                if (stock.qty > 0) {
                    if (stock.qty > 0) {
                        return {
                            label: stock.size,
                            value: stock.size,
                            selected: stock.size === selectedSize,
                        };
                    }
                }
            }
        );
        const dataOptions = options?.filter((option) => option !== undefined);
        return dataOptions;
    };
    ...
    return (
        <div className={styles.cart}>
                        ...
                        <Fragment key={`${item.id}-${item.size}`}>
                            <div className={styles.cart__main__list__item}>
                                ...
                                <div className={styles.cart__main__list__item__info}>
                                    ...
                                    >
                                        <i className="bx bx-trash" /> <span style={{ display: "none" }}>Delete</span>
                                    </button>
                                </div>
                                <h4 className={styles.cart__main__list__item__info__price}>
            ...
            <div className={styles.cart__summary}>
                ...
                <hr />
                <Button type="button" className={styles.cart__summary__button}>Checkout</Button>
            </div>
        </div>
    );
};
